...
//fixme: missing: '+' (master voice) and '*' (floating voice)
	...
	function sym_dump(s) {
		...
		case STBRK:
			voice_out();
			print('%%staffbreak ' + s.xmx.toString() +
				(s.stbrk_forced ? 'f' : ''))
			break
		case TEMPO:
			tempo_dump(s)
	...
	print(meter_dump(voice_tb[0].meter));

	ulen = voice_tb[0].ulen < 0 ? BASE_LEN / 4 : voice_tb[0].ulen;
	print('L:1/' + (BASE_LEN / ulen).toString());
	...
	print(key_dump(voice_tb[0].key, voice_tb[0].clef))
